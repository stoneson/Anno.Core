...
namespace NetTools
{
...
#if NET40
    [Serializable]
    public class IPAddressRange : ISerializable, IEnumerable<IPAddress>, IDictionary<string, string>, IEquatable<IPAddressRange>
#elif NET45
    [Serializable]
...
#endif
    {
        ...
        {
            get { return TryGetValue(key, out var value) ? value : throw new KeyNotFoundException(); }
            set { }
        }
        ICollection<string> IDictionary<string, string>.Keys => GetDictionaryItems().Select(item => item.Key).ToArray();

        ICollection<string> IDictionary<string, string>.Values => GetDictionaryItems().Select(item => item.Value).ToArray();

        bool IDictionary<string, string>.ContainsKey(string key) => GetDictionaryItems().Any(item => item.Key == key);

        void IDictionary<string, string>.Add(string key, string value) { return; }

        bool IDictionary<string, string>.Remove(string key) { return true; }

        bool IDictionary<string, string>.TryGetValue(string key, out string value) => TryGetValue(key, out value);


        int ICollection<KeyValuePair<string, string>>.Count => GetDictionaryItems().Count();
        ...
        void ICollection<KeyValuePair<string, string>>.Add(KeyValuePair<string, string> item) { return; }

        void ICollection<KeyValuePair<string, string>>.Clear() { return; }

        bool ICollection<KeyValuePair<string, string>>.Contains(KeyValuePair<string, string> item)
            => GetDictionaryItems().Any(k => item.Key == k.Key);

        void ICollection<KeyValuePair<string, string>>.CopyTo(KeyValuePair<string, string>[] array, int arrayIndex)
        {
            
        }
        bool ICollection<KeyValuePair<string, string>>.Remove(KeyValuePair<string, string> item) { return false; }
#endif
    }
}
